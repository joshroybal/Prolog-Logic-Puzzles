...
start(TitleNames, Types, PenNames, IdeasFrom) :-
    ...
    fd_domain(Types, 1, 5), fd_all_different(Types),
    ...
    fd_domain(PenNames, 1, 5), fd_all_different(PenNames),

    IdeasFrom = [AgathaChristie, CatherineCookson, ConanDoyle, IanFleming,
		 RudyardKipling],
    fd_domain(IdeasFrom, 1, 5), fd_all_different(IdeasFrom),
    ...
    fd_labeling(Types),
    fd_labeling(PenNames),
    fd_labeling(IdeasFrom),    
